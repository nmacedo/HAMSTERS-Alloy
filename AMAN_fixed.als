...
one sig ManageLandingSequenceLS extends Concurrent {} {
	subtasks = 0->MonitorLS + 1->ChangeZoom + 2->ModifyLS + 3->BlockTimeSlot + 4->SelectAircraftLabel + 5->ClickHoldButton // Fix: pull hold tasks up
}
...
// Fix: removed PutAircraftOnHold, pulled subtasks up
one sig SelectAircraftLabel extends Atomic {} {
	// Can only occur if there is some displayed plane that is not on hold 
	guard = True iff (some (Slot-holding).label - selected) // Fix: can only select non-selected
}
one sig ClickHoldButton extends Atomic {} {
	// Can only occur if exactly one plane is selected
	guard = True iff one selected & Plane & displayed.label // Fix: can only press hold if selected visible
}
...
fact {
	Iterative = ManageLS
	Optional = ChangeZoom+ModifyLS+BlockTimeSlot+SelectAircraftLabel+ClickHoldButton // Fix: pull hold tasks up
...
pred selectAircraftLabel {
	...
	some s : (Slot-holding).label-selected | Plane <: selected' = s // Fix: can only select non-selected
...
// Scenarios 
// Fix: scope of 6 seq after hold tasks pulled up
